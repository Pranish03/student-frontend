...
import { LuChevronRight, LuFileText, LuClipboardList } from "react-icons/lu";
...
export const ManageResources = () => {
  ...
  return (
    <Container>
      ...
      <div className="mb-8">
        <Heading className="mb-1">Manage Resources</Heading>
        <Paragraph>
          {courses.length} course{courses.length !== 1 ? "s" : ""} — upload
...
const CourseCard = ({ course }) => {
  const { data: noteData } = useQuery({
    queryKey: ["notes", course._id],
    queryFn: () =>
      axios
        .get(`/resources/course/${course._id}?type=note`)
        .then((r) => r.data)
        .catch((e) => {
  ...
  const noteCount = noteData?.resources?.length ?? null;
  const assignmentCount = assignmentData?.resources?.length ?? null;
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <div className="flex items-center gap-3 mb-4">
          <div className="flex items-center gap-1.5 text-sm text-zinc-500">
            ...
            {noteCount === null ? (
              <span className="w-12 h-3 bg-zinc-100 rounded animate-pulse inline-block" />
            ) : (
              <span>
          ...
          <div className="flex items-center gap-1.5 text-sm text-zinc-500">
            <LuClipboardList size={14} className="text-orange-500" />
            {assignmentCount === null ? (
              <span className="w-20 h-3 bg-zinc-100 rounded animate-pulse inline-block" />
            ) : (
              <span>
        ...
        <Link to={`/teacher/manage-resources/${course._id}`}>
