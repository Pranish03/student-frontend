...
export const ManageNotes = () => {
  ...
  return (
    <Container>
      ...
      <div className="mb-8">
        <Heading className="mb-1">Manage Notes</Heading>
        <Paragraph>
...
const CourseCard = ({ course }) => {
  ...
  const noteCount = resourceData?.resources?.length ?? null;
  ...
  return (
    <div className="bg-white rounded-[10px] overflow-hidden border border-zinc-300 flex flex-col">
      <div className="p-6 flex flex-col flex-1">
        ...
        <Link to={`/teacher/manage-notes/${course._id}`}>
          <Button className="w-full">Upload or Browse Notes</Button>
        </Link>
      </div>
    </div>
  );
